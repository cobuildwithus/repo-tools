...
extra_tracked_paths="${COBUILD_DOC_GARDENING_EXTRA_TRACKED_PATHS:-}"
tracked_docs="$({
  ...
  find agent-docs/references -type f -name '*-llms.txt' 2>/dev/null || true
  if [[ -n "$extra_tracked_paths" ]]; then
    printf '%s\n' "$extra_tracked_paths"
  fi
} | sed '/^[[:space:]]*$/d' | sort -u)"

indexed_paths="$({
  awk -F'`' '/^\|/ && $2 ~ /^(agent-docs\/|ARCHITECTURE\.md$)/ {print $2}' "$INDEX_FILE"
} | sort -u || true)"
...

{
  ...
  echo
  echo "Auto-generated by cobuild-doc-gardening."
  echo
  echo "## Tracked Artifacts (.md + references/*-llms.txt + configured extras)"
